...
auto mult=control(L"COMBOBOX",L"",Multiplier,CBS_DROPDOWNLIST,1150,10,70,180);for(auto label:{L"2X",L"3X",L"4X",L"5X",L"6X"})SendMessageW(mult,CB_ADDSTRING,0,reinterpret_cast<LPARAM>(label));SendMessageW(mult,CB_SETCURSEL,initialOptions.fgMultiplier-2,0);
...
case Play:{auto s=engine.snapshot();if(s.capture||s.image||s.transport==veyra::engine::TransportState::Opening||s.transport==veyra::engine::TransportState::Stopping)break;if(!currentFile.empty()&&(!s.running||s.transport==veyra::engine::TransportState::Ended||s.failed||s.transport==veyra::engine::TransportState::Failed)){openFile(currentFile);break;}if(seekPreview.active){seekPreview.resume=!seekPreview.resume;paused=!seekPreview.resume;break;}paused=s.transport==veyra::engine::TransportState::Playing;engine.pause(paused);SetWindowTextW(GetDlgItem(hwnd,Play),paused?L"播放":L"暂停");break;}
...
setText(GetDlgItem(hwnd,TimeLabel),s.remotePlay?(s.remoteRatesReady?std::format(L"接收 {:.1f} · 解码 {:.1f} fps",s.remoteReceivedFps,s.remoteDecodedFps):std::wstring(L"PS5 帧率采样中")):s.capture?std::format(L"输入 {:.1f} fps{}",s.captureFps,s.captureHalfRate?L" · 60→30":s.applied.content==veyra::engine::ContentRate::Capture60To30?L" · 不适用":L""):s.image?L"静态图片":stamp(s.seekPresented<s.seekRequested&&!s.failed?s.seekTarget:s.position)+L"  /  "+stamp(s.duration)+(s.seekPresented<s.seekRequested&&!s.failed?L"  ·  跳转中…":L"")+(s.transport==veyra::engine::TransportState::Opening?L"  ·  正在打开…":s.failed?L"  ·  发生错误，点播放重试或见专业诊断":L""));
...
case WM_COPYDATA:{auto cds=reinterpret_cast<COPYDATASTRUCT*>(lp);if(!cds||cds->dwData!=0x31534656u||!cds->lpData||cds->cbData<sizeof(wchar_t)||cds->cbData>32768*sizeof(wchar_t))return 0;std::wstring file(static_cast<const wchar_t*>(cds->lpData),cds->cbData/sizeof(wchar_t));while(!file.empty()&&file.back()==L'\0')file.pop_back();if(file.empty()||file.find(L'\0')!=std::wstring::npos)return 0;if(IsIconic(hwnd))ShowWindow(hwnd,SW_RESTORE);SetForegroundWindow(hwnd);openFile(file);layout();return 1;}
case WM_DESTROY:
#ifdef VEYRA_ENABLE_REMOTEPLAY
