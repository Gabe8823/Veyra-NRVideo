...
inline LRESULT colors(UINT msg,WPARAM w,LPARAM l=0){HDC dc=reinterpret_cast<HDC>(w);HWND h=reinterpret_cast<HWND>(l);wchar_t cls[32]{};GetClassNameW(h,cls,32);const bool edit=_wcsicmp(cls,L"EDIT")==0;COLORREF bg=edit?raised:surface(h);SetTextColor(dc,edit?textColor:secondary);SetBkColor(dc,bg);SetBkMode(dc,TRANSPARENT);if(edit&&GetPropW(h,L"Veyra.EditPrint")){SetBkColor(dc,RGB(0,0,0));return reinterpret_cast<LRESULT>(GetStockObject(BLACK_BRUSH));}SetDCBrushColor(dc,bg);return reinterpret_cast<LRESULT>(GetStockObject(DC_BRUSH));}
...
inline LRESULT CALLBACK buttonPaint(HWND h,UINT m,WPARAM w,LPARAM l,UINT_PTR,DWORD_PTR){
    ...
    if(controlVisualChange(m)||m==BM_SETCHECK||m==BM_SETSTATE||m==BM_SETSTYLE)return updateControlModel(h,m,w,l);
