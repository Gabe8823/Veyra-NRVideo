...
struct Shared {
    DWORD signature=magic,version=1,bytes=sizeof(Shared);
...
const wchar_t* label(ExportState s){switch(s){case ExportState::Preparing:return L"正在准备独立导出任务";case ExportState::Running:return L"正在编码 · 原生NR · D3D12 NVENC";case ExportState::Paused:return L"导出已暂停，观看继续";case ExportState::Finishing:return L"正在封装和验证输出";case ExportState::Succeeded:return L"导出完成，输出解码验证通过";case ExportState::Failed:return L"导出失败；详见独立任务日志，partial已保留";case ExportState::Cancelled:return L"导出已取消；已写入的partial保留";default:return L"尚无导出任务";}}
}
...
bool ExportJobManager::start(const std::wstring& input,const std::wstring& output,EnhancementSettings settings,bool hevc,unsigned maxFrames){
    ...
    log::info("export-worker",std::format("started pid={} frozenRevision={} nativeNR=true independentPlayback=true",pi.dwProcessId,settings.revision));return true;
}
